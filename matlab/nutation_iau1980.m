function [eps, deps, dpsi] = nutation_iau1980(JD)
% nutation - Compute nutation parameters based on the IAU 1980 Nutation
% Model
%
% INPUTS:
%   JD         Julian Date
%
% OUTPUTS:
%   eps        Nutation parameter eps (in degrees)
%   deps       Nutation parameter \Delta eps (in degrees)
%   dpsi       Nutation parameter \Delta psi (in degrees)
%
% References:
...
eps = 23.4392911111 - 0.0130041667 * T - 1.6388888889e-07 * T2 + 5.0361111111e-07 * T3;

% Mean anomaly of the Moon (A.26):
a_1 = 134.9629813889 + (198.8673980555 * T) + (8.6972222222e-3 * T2) + (1.7777777778e-05 * T3) ...
     + mod(1325.0 * 360.0 * T, 360.0);
% Mean anomaly of the Sun (A.27):
a_2 = 357.5277233333 + (359.0503400000 * T) - (1.6027777778e-4 * T2) - (3.3333333333e-06 * T3) ...
     + mod(99.0 * 360.0 * T, 360.0);
% Moon's mean argument of latitude (A.28):
a_3 =  93.2719102778 + ( 82.0175380556 * T) - (0.0036825000000 * T2) + (3.0555555555e-06 * T3) ...
     + mod(1342.0 * 360.0 * T, 360.0);
% Moon's mean elongation from the Sun (A.29):
a_4 = 297.8503630555 + (307.1114800000 * T) - (0.0019141666667 * T2) + (5.2777777778e-06 * T3) ...
     + mod(1236.0 * 360.0 * T, 360.0);
% Mean longitude of the ascending lunar node (A.30):
a_5 = 125.0445222222 - (134.1362608333 * T) + (0.0020708333333 * T2) + (2.2222222222e-06 * T3) ...
     - mod(5.0 * 360.0 * T, 360.0);
...
factor = 1.0/(10000.0 * 3600.0);
        
[N, cols] = size(nut_data);
dpsi = zeros(1, num_dates);
deps = zeros(1, num_dates);
...
for j = 1:N
    ...
    angle = data(KJ1) * a_1 + data(KJ2) * a_2 + data(KJ3) * a_3 + data(KJ4) * a_4 + data(KJ5) * a_5;
